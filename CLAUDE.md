...
| 问题 | 文件 | 描述 | 优先级 |
|------|------|------|--------|
| C1 | ecs.go:109-168 | CreateECSInstance 从快照创建临时镜像，若 CreateInstance 失败会导致镜像泄漏，需返回 cleanup 函数 | P1 |
| C2 | ecs.go:241,344 | DescribeECSInstance 和 WaitForInstanceRunning 缺少 InstanceId 的 nil 检查 | P2 |
| W1 | vpc.go:64-83 | WaitVPCAvailable 不支持 context 取消，与其他等待函数风格不一致 | P2 |
