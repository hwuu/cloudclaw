...
// Credentials 阿里云凭证，从 ~/.cloudclaw/credentials 文件加载
...
// LoadCredentials 从 ~/.cloudclaw/credentials 文件加载凭证。
...
func LoadCredentialsFrom(path string) (*Credentials, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, fmt.Errorf("凭证文件不存在，请先运行 cloudclaw init")
		}
	...
	if cred.AccessKeyID == "" {
		return nil, fmt.Errorf("凭证文件缺少 access_key_id，请运行 cloudclaw init 重新配置")
	}
	if cred.AccessKeySecret == "" {
		return nil, fmt.Errorf("凭证文件缺少 access_key_secret，请运行 cloudclaw init 重新配置")
	}

	return cred, nil
}

// SaveCredentials 将凭证保存到 ~/.cloudclaw/credentials，权限 600
