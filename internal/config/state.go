// Package config 管理 CloudClaw 的持久化状态和用户交互。
...
// GatewayConfig Gateway 配置（域名、Token 等）
type GatewayConfig struct {
	Domain string `json:"domain"`
	Token  string `json:"token"`
}

// State 部署状态，序列化为 ~/.cloudclaw/state.json
type State struct {
...
// GetStateDir 返回状态目录路径（~/.cloudclaw/）
...
// GetStatePath 返回状态文件完整路径（~/.cloudclaw/state.json）
