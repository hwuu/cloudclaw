...
// 删除顺序：解绑 EIP → 释放 EIP → 删除 ECS → 删除 SSH 密钥对 → 删除安全组 → 删除 VSwitch → 删除 VPC。
...
type Destroyer struct {
	...
	Region       string
	StateDir     string
	Version      string        // CloudClaw 版本号（写入 backup.json）
...
func (d *Destroyer) Run(ctx context.Context, force, dryRun bool) error {
	...
	d.printf("\n✅ 所有资源已清理完毕。\n")
	if keepSnapshot {
		d.printf("  快照已保留，下次 cloudclaw deploy 可从快照恢复。\n")
	}
	return nil
}
...
func (d *Destroyer) createSnapshot(ctx context.Context, state *config.State) error {
	...
	// 创建快照
	d.printf("  创建快照...\n")
	snapshotName := fmt.Sprintf("cloudclaw-%s", time.Now().UTC().Format("20060102-150405"))
