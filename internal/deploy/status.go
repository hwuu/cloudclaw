...
func (s *StatusRunner) Run(ctx context.Context) error {
	...
	if err != nil {
		s.printf("未找到部署记录。请先运行 cloudclaw deploy\n")
		return nil
	}

	s.printf("CloudClaw 部署状态\n")
	s.printf("─────────────────────────────────────────\n")
	...
	// 应用信息
	if state.Gateway.Domain != "" {
		s.printf("\n应用:\n")
		s.printf("  %s %s\n", padRight("域名:", 12), state.Gateway.Domain)
		s.printf("  %s https://%s\n", padRight("地址:", 12), state.Gateway.Domain)
	}

	// 容器状态（通过 SSH）
	if state.Status == "suspended" {
		s.printf("\n状态：已停机 (suspended)\n")
		s.printf("  恢复运行：cloudclaw resume\n")
	} else if state.Status == "destroyed" {
		s.printf("\n状态：已销毁 (destroyed)\n")
		s.printf("  重新部署：cloudclaw deploy\n")
